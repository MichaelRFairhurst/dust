...
/// [Path]s, the time it takes to run, and whether it the fuzz case failed.
///
...
///
/// Assuming the uniqueness of the paths have already been scored, it's easy
/// for us to factor in time. Running a seed that takes twice as long but has
/// twice the unique paths is likely to be a wash: S(l) / t.
///
...
/// contains paths not seen in any fuzz successes. Or the tolerance of
...
///
/// Note that the [PathScorer] will change scores over time, and therefore,
/// this will too.
class SeedScorer {
  final PathScorer _pathScorer;

  /// Initialize a [SeedScorer] with a custom [PathScorer].
  SeedScorer(this._pathScorer);
  ...
  double _pathScore(Seed seed) =>
      _pathScorer.scoreAll(seed.initialResult.paths);
